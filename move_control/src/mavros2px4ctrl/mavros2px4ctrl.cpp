...
void mavros2px4::setParameters() {
    ...
    nh_private_.param<float>("Kp_x", Kp_x_, 7);
    nh_private_.param<float>("Kp_y", Kp_y_, 7);
    nh_private_.param<float>("Kp_z", Kp_z_, 10);
    nh_private_.param<float>("Kv_x", Kv_x_, 1.5);
    nh_private_.param<float>("Kv_y", Kv_y_, 1.5);
    nh_private_.param<float>("Kv_z", Kv_z_, 3);
    nh_private_.param<float>("norm_thrust_const", norm_thrust_const_, 0.03);

    controlLer.setParameters();
    controlLer.setProportionParams(Kp_x_,Kp_y_,Kp_z_,Kv_x_,Kv_y_,Kv_z_);
...
void mavros2px4::mainloop(const ros::TimerEvent &event) {
    ...
    switch (mode.data) {
        ...
        case POSTION_CONTROL:
            ...
            if ((now_time-cmd_received_time).toSec()>cmd_delay_bear_time)
            {   //  if cmd is not received in cmd_delay_bear_time, trun to HOVER mode;
                ROS_INFO_STREAM("\033[31m Turn to HOVER \033[0m");
                mode.data=HOVER;
                break;
            }
        ...
        case NO_ACTION:
            ...
            if(mode_last.data!= NO_ACTION)
            {
                ROS_INFO_STREAM("\033[33m NO ACTION NOW \033[0m");
            }
            mode_last.data=NO_ACTION;
    ...
    if(mode.data == ATTITUDE_CONTROL)             //  pub control msg to px4
    {
