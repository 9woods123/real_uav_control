...
int main(int argc, char **argv)
{
    ...
    float dt=0.25;
    float yaw=0;
    float mav_velocity=1;
    for (int i = 0; i <= 30; i++)
    {
        std::cout << "========move=======" << std::endl;

        Eigen::Vector3d Vel(0,mav_velocity,0);

        Eigen::Vector3d Pos(fcu_position.pose.position.x,
                            fcu_position.pose.position.y+mav_velocity*dt,
                            fcu_position.pose.position.z);

        Eigen::Vector3d Acc(0,0,0);

        yaw+=0.1;
        send_pos_setpoint(Pos,Vel,Acc,getyawfrompose(fcu_position));
